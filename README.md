...
2. «Мои группы» → добавить барахолки из меню.
3. «Добавить объявление» → текст → фото → цена → «Запустить».
4. «Автопостинг» → включить.

**Без авторизации аккаунта (по умолчанию):** бот по расписанию присылает готовый пост в личку — пересылаешь в группу.

**Полный автопост (опционально):** «Мой аккаунт» → QR (как Desktop). Номер/SMS боту не нужны.

> Автопостинг стабильно работает через `python main.py`. На Vercel планировщик слабый.
